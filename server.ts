...
const getNoaaWeatherData = async (lat: number, lon: number) => {
  ...
  if (daytimePeriods.length < 7 || !hourlySlice.length) {
    throw new Error('NOAA returned an incomplete seven-day forecast');
  }
  ...
  const dailyMin = daytimePeriods.map((period: any, index: number) => {
    const night = periods.find((item: any) => !item.isDaytime && item.number > daytimePeriods[index].number);
    return Number(night?.temperature || dailyMax[index] - 10);
  });
  const firstDate = new Date(`${daytimePeriods[0].startTime.slice(0, 10)}T00:00:00Z`);
  firstDate.setUTCDate(firstDate.getUTCDate() - 1);
  const priorDate = firstDate.toISOString().slice(0, 10);
  const priorTemperature = dailyMax[0];
  ...
  return {
    ...
    daily: {
      time: [priorDate, ...daytimePeriods.map((period: any) => period.startTime.slice(0, 10))],
      weather_code: [weatherCodeFor(daytimePeriods[0].shortForecast || ''), ...daytimePeriods.map((period: any) => weatherCodeFor(period.shortForecast || ''))],
      temperature_2m_max: [priorTemperature, ...dailyMax],
      temperature_2m_min: [dailyMin[0], ...dailyMin],
      precipitation_sum: [0, ...daytimePeriods.map(() => 0)],
      precipitation_probability_max: [0, ...daytimePeriods.map((period: any) => Number(period.probabilityOfPrecipitation?.value || 0))],
      wind_speed_10m_max: [windMph(daytimePeriods[0].windSpeed), ...daytimePeriods.map((period: any) => windMph(period.windSpeed))],
    },
    timezone: 'auto',
    elevation: 0,
  };
    ...
    try {
      ...
      if (lat >= 24 && lat <= 50 && lon >= -125 && lon <= -66) {
        try {
          ...
          console.warn('[Weather] Open-Meteo unavailable; using NOAA seven-day backup:', error);
        } catch (noaaError) {
